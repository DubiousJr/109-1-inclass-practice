...
```{r ans13}
# G5TimeParsed
Data<-c("Date","StartTime","TimeZone")
Time<-c(Game5Time[[1]],
Game5Time[[2]],
Game5Time[[3]])
...
```{r ans14}
# TpeJason
Allen <- mdy_hms("Oct.9, 2020, 21:00:00",
                 tz="America/New_York")
TpeJason<-with_tz(Allen,
                  tz="Asia/Taipei")
Allen
TpeJason
```
...
```{r ans15}
# ParisLouis
ParisLouis<-with_tz(Allen,
                  tz="Europe/Paris")
lubridate::minutes(ParisLouis+900)
as.character(ParisLouis+900)
...
as.numeric(chNumbers_factor)
sum(as.numeric(chNumbers_factor))
...
sumChNumberswith0<-
sum(as.numeric(chNumbersWith0_factor))-length(chNumbersWith0_factor)
sumChNumberswith0
```
...
info1<-c(student$name)
info2<-c(student$deadline)
dialogue1<-c("Dear")
dialogue2<-c(":")
dialouge1.5<-c("。")
dialogue3<-c("貼心提醒您，作業上傳截止時間是")
dialogue2.5<-paste0(dialogue3,"",info2,"",dialouge1.5)
dialogue4<-c("敬祝學安")
dialogue5<-c("你的貼心學伴巴西利bot silly")
dialogueFinal<-paste0(info1,"",dialogue2,"
...
",dialogue5)
dialogue<-paste0(dialogue1," ",dialogueFinal)

dearStudent<-c(dialogue)
cat(dearStudent)
```
