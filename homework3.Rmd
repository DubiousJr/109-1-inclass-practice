...
table(User_Counts_with_gender_NA)
```
...
```{r ans13}
# Users_E
pick_grade<-Info_UserData$user_grade<=6
Users_E<-Info_UserData$uuid[pick_grade&User_Counts_with_gender_NA]
...
first<-Info_UserData$user_grade==1
third<-Info_UserData$user_grade==3
fifth<-Info_UserData$user_grade==5
pick_grade2<-first|third|fifth
Users_E_in_odd_grades<-Info_UserData$uuid[pick_grade2&User_Counts_with_gender_NA]
Users_E_in_odd_grades
```
...
pick_Metro<-Info_UserData$user_city %in% set_Metro
pick_Not_Metro<- ! pick_Metro
Users_E_Not_Metro<-Info_UserData$uuid[pick_grade&User_Counts_with_gender_NA&pick_Not_Metro]
Users_E_Not_Metro
```
...
login_date<-Info_UserData$first_login_date_TW==107
not_Effected<-login_date|Info_UserData$is_self_coach
Users_E_Not_Metro_Buffer<-Info_UserData$uuid[pick_grade&User_Counts_with_gender_NA&pick_Not_Metro&not_Effected]
Users_E_Not_Metro_Buffer
```
...
best_county_votes<-election2020$有效票數[election2020$`鄉(鎮、市、區)別`=="貢寮區"]
best_county_votes

best_ddp_votes=best_county_votes*0.6679
best_ddp_votes
...
```{r ans22}
# levelSequence
ddpSupportRate<-election2020$`(3)
 蔡英文
 賴清德`

dppSupportRate_sorted<-sort(ddpSupportRate)


levelSequence[order(factor(names(dppSupportRate_sorted),levels = c(election2020$`鄉(鎮、市、區)別`)))]

class(ddpSupportRate)

levelSequence

f_levelsequence<-factor(levelSequence)
...
election2020[["鄉(鎮、市、區)別"]]<-factor(election2020$`鄉(鎮、市、區)別`,
  levels=rev(levelSequence))

class(election2020[["鄉(鎮、市、區)別"]])
